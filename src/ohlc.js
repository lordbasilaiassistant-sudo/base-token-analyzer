// OHLC bar builder + technical levels + buy/sell signals.

const { rawPriceFromSqrtX96, priceWethPerToken } = require('./v4');

// Base mainnet: block 0 at 2023-06-15 12:35:47 UTC. Block time = 2s.
const BASE_GENESIS_TS = 1686789347;
const BASE_BLOCK_TIME = 2;

// Compute timestamps locally for a list of blocks (Base has fixed 2s blocks).
// Optionally calibrate against one real block to absorb genesis-offset drift.
async function fetchBlockTimestamps(rpc, blocks, _concurrency, { calibrate = true } = {}) {
  const map = new Map();
  let offset = 0;
...
// Convert raw V4 swaps into trade rows with timestamp, priceWeth, volWeth, side.
function swapsToTrades(swaps, init, tokenAddress, decimalsToken, blockTs) {
  ...
  return swaps.map(s => {
    ...
    // amount0 / amount1 are signed int128; positive means pool gained (user paid in)
    const a0 = s.amount0;
    const a1 = s.amount1;
    const tokenAmount = isToken0 ? a0 : a1;
    const wethAmount  = isToken0 ? a1 : a0;
    // From trader perspective: tokenAmount > 0 means trader sold token (pool gained token).
...
// Bucket trades into candles. interval in seconds.
...
// Pivot-based support/resistance: a high is a "swing high" if it's higher than the N candles either side.
...
// Volume-weighted price levels — bucket prices into N bins, sum volume per bin.
...
module.exports = { fetchBlockTimestamps, swapsToTrades, buildCandles, findPivots, volumeProfile, sma, atr };
