// DexScreener public API — no auth, ratelimited but generous.
...
// Pick the best (highest-liquidity) Base pair, optionally filtered by pairAddress (poolId for V4).
function pickBestPair(json, opts = {}) {
  const pairs = (json && json.pairs) || [];
  const onBase = pairs.filter(p => p.chainId === 'base');
  let candidates = onBase;
  if (opts.pairAddress) {
...
module.exports = { fetchToken, pickBestPair, summarizePair };
